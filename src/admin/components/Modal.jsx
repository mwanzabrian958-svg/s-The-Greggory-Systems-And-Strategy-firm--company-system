...
export function Modal({ 
  ...
  return (
    ...
    >
      ...
      <div 
        ref={modalRef}
        className={`relative w-full ${sizeClasses[size]} bg-white dark:bg-[#032457] shadow-2xl ${isScreen ? 'h-screen rounded-none' : 'rounded-3xl border border-gray-100 dark:border-white/5'} overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col`}
      >
        {/* Header */}
        {title && (
          <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-white/5 flex-shrink-0">
            ...
            {showCloseButton && (
              ...
              >
                <X className="w-6 h-6 text-slate-400 dark:text-zinc-500 group-hover:text-rose-500 transition-colors" />
              </button>
            )}
          </div>
        )}
