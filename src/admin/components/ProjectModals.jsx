    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Initialize Mission Node" size="xl">
      <form onSubmit={handleSubmit} className="space-y-6 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div className="flex gap-4 pt-6 border-t border-slate-50">
          ...
          <button type="submit" disabled={loading} className="flex-[2] px-6 py-4 rounded-2xl bg-blue-700 text-white text-[10px] font-black uppercase tracking-widest hover:bg-blue-800 shadow-xl shadow-blue-900/20 disabled:opacity-50 transition-all">
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Recalibrate Node: ${project?.project_name}`} size="xl">
      <form onSubmit={handleSubmit} className="space-y-6 max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div className="space-y-3">
          ...
          <input type="range" min="0" max="100" value={formData.progress_percentage} onChange={(e) => setFormData({...formData, progress_percentage: e.target.value})} className="w-full h-1.5 bg-slate-100 rounded-full appearance-none cursor-pointer accent-blue-700" />
        </div>
        ...
        <div className="bg-slate-50 p-4 rounded-2xl space-y-3">
            ...
            {team.map((m) => (
              <div key={m.id} className="flex items-center justify-between bg-white p-2.5 rounded-xl border border-slate-100">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-blue-500 to-emerald-600 flex items-center justify-center text-white text-[8px] font-black">
          ...
          <div className="grid grid-cols-[1fr_120px_auto] gap-2 pt-2">
            ...
            />
            <button type="button" onClick={handleAssign} className="mt-4 p-2 bg-blue-700 text-white rounded-xl hover:bg-blue-800 transition-colors"><Plus size={14} /></button>
          </div>
        </div>
        ...
        <div className="flex gap-4 pt-6 border-t border-slate-50">
          ...
          <button type="submit" disabled={loading} className="flex-[2] px-6 py-4 rounded-2xl bg-blue-700 text-white text-[10px] font-black uppercase tracking-widest hover:bg-blue-800 shadow-xl shadow-blue-900/20">
