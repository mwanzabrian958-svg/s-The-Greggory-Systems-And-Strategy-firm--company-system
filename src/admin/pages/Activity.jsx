...
export function ActivityLogs() {
  ...
  if (loading) return <div className="flex items-center justify-center py-20"><RefreshCw className="animate-spin text-blue-700 w-6 h-6" /></div>;
    ...
    <div className="space-y-6 animate-fade-in font-sans w-full">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
            <button onClick={fetchLogs} className="p-2 bg-white border border-slate-100 rounded-lg text-blue-700 shadow-sm"><RefreshCw size={14} /></button>
        </div>
      </div>

      {/* Tighter Log Controls */}
      <div className="bg-[#00122B] rounded-xl p-3 border border-white/10 shadow-xl flex flex-col md:flex-row justify-between items-center gap-3">
        ...
        <div className="flex gap-3">
           <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-[8px] font-black uppercase text-white outline-none">
              <option value="all" className="bg-[#00122B]">All Activities</option>
              <option value="auth" className="bg-[#00122B]">Authentication</option>
              <option value="ledger" className="bg-[#00122B]">Financial</option>
              <option value="content" className="bg-[#00122B]">Content</option>
           </select>
        </div>
      </div>
                  ...
                  <tr key={i} className="hover:bg-slate-50/50 transition-all">
                     ...
                     <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                           <div className="w-5 h-5 rounded-full bg-[#00122B] flex items-center justify-center text-[7px] font-black text-white">{(l.admin_name || 'S')[0]}</div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-6 pb-12">
          ...
          <div className="px-4 py-2 bg-[#00122B] rounded-xl text-[8px] font-black text-white uppercase tracking-widest border border-white/10 shadow-lg">Log {currentPage} / {totalPages}</div>
