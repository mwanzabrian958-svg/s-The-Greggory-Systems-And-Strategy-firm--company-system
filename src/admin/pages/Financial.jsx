...
export function Billing() {
  ...
  return (
    <div className="space-y-6 animate-fade-in font-sans w-full pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => navigate('/admin/billing/entry')} className="flex items-center gap-2 bg-white border border-slate-200 hover:border-blue-500 text-slate-700 rounded-xl px-5 py-3 text-[9px] font-black uppercase tracking-widest transition-all">
      ...
      {/* SECTION 2: TABS */}
      <div className="bg-[#00122B] rounded-2xl p-4 border border-white/10 shadow-xl flex justify-between items-center">
         <div className="flex gap-2">
            {[
              ...
              <button
                ...
                className={`px-6 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${displayMode === t.key ? 'bg-blue-700 text-white shadow-lg shadow-blue-900/40' : 'bg-white/5 text-slate-400 hover:text-white'}`}
              >
                {t.label}
              </button>
            ))}
         </div>
         <button onClick={fetchFinancialData} className="text-[7px] font-black text-blue-400 uppercase tracking-widest flex items-center gap-1 hover:text-white transition-colors">
                ...
                <tbody className="divide-y divide-slate-50">
                  {financials.invoices.map(inv => (
                    <tr key={inv.id} className="hover:bg-slate-50/50 transition-all">
                      <td className="py-3 px-2 text-[9px] font-black text-blue-800 font-mono">{inv.invoice_number || `INV-${inv.id}`}</td>
                      ...
                      <td className="py-3 px-2">
                        <div className="flex gap-1 justify-end">
                          <button onClick={() => handleSendInvoice(inv)} disabled={sendingId === inv.id} title={inv.client_email ? `Send invoice to ${inv.client_email}` : "No client email"} className={`p-1.5 rounded-lg transition-all ${inv.email_sent ? "bg-emerald-50 text-emerald-600" : "bg-slate-50 text-slate-400 hover:bg-blue-500 hover:text-white"} disabled:opacity-60`}>
                            {sendingId === inv.id ? <RefreshCw size={11} className="animate-spin" /> : <Send size={11} />}
                          </button>
                          <button onClick={() => handleDownloadCompletion("invoices", inv.id)} title="Download completion PDF" className="p-1.5 bg-slate-50 rounded-lg text-slate-400 hover:bg-blue-700 hover:text-white transition-all"><FileDown size={11} /></button>
                          <button onClick={() => navigate(`/admin/billing/preview/${inv.id}`)} title="View invoice" className="p-1.5 bg-slate-50 rounded-lg text-slate-400 hover:bg-blue-500 hover:text-white transition-all"><Eye size={11} /></button>
        ...
        {displayMode === "ledger" && (
          <div className="p-6">
            <div className="flex justify-between items-center mb-6">
               ...
               <button onClick={() => navigate('/admin/billing/entry')} className="text-[8px] font-black text-blue-700 uppercase tracking-widest flex items-center gap-1 hover:underline">
                  ...
                  {financials.entries.map(e => (
                    <tr key={e.id} className="hover:bg-slate-50/50 transition-all">
                      ...
                      <td className="py-3 px-2 text-right">
                        <button onClick={() => handleDownloadCompletion("accounting_entries", e.id)} title="Download completion PDF" className="p-1.5 bg-slate-50 rounded-lg text-slate-400 hover:bg-blue-700 hover:text-white transition-all mr-1"><FileDown size={11} /></button>
                  ...
                  {financials.entries.length === 0 && (
                    <tr>
                      <td colSpan="7" className="py-14 text-center">
                        ...
                        <button onClick={() => navigate('/admin/billing/entry')} className="bg-blue-700 hover:bg-blue-800 text-white rounded-xl px-6 py-2.5 text-[8px] font-black uppercase tracking-widest transition-all">
                          Record first entry
                        </button>
                      </td>
                    </tr>
