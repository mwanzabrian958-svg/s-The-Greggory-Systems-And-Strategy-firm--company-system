...
export function AdvancedDashboard({ user }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in font-sans w-full transition-colors duration-300">
      {/* Welcome Node */}
      <div className="bg-[#00122B] rounded-2xl p-6 border border-white/10 shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-48 h-48 bg-[#002D62]/20 blur-[80px] -mr-24 -mt-24"></div>
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-black text-white uppercase tracking-tight">Welcome, <span className="text-blue-400">{userName}</span></h1>
          ...
          <div className="flex gap-3">
            <button onClick={() => navigate('/admin/billing')} className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 hover:bg-white/10 transition-all text-left group">
              <p className="text-[6px] text-slate-500 font-black uppercase tracking-widest group-hover:text-blue-400">Revenue</p>
            ...
            <button onClick={() => navigate('/admin/billing/pl-report')} className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 hover:bg-white/10 transition-all text-left group">
              <p className="text-[6px] text-slate-500 font-black uppercase tracking-widest group-hover:text-blue-400">Net Income</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "New Invoice", icon: DollarSign, path: "/admin/billing/create", tone: "text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10" },
          { label: "Add Personnel", icon: UserCheck, path: "/admin/users", tone: "text-sky-600 bg-sky-50 dark:bg-sky-500/10" },
          { label: "Team Management", icon: Users, path: "/admin/team", tone: "text-violet-600 bg-violet-50 dark:bg-violet-500/10" },
          { label: "Data Safety", icon: Shield, path: "/admin/data-safety", tone: "text-rose-600 bg-rose-50 dark:bg-rose-500/10" },
        ].map((a) => (
          <button key={a.label} onClick={() => navigate(a.path)} className="bg-white dark:bg-[#032457] rounded-xl p-3 border border-slate-100 dark:border-white/5 shadow-sm flex items-center gap-3 hover:border-[#002D62] dark:hover:border-blue-500/40 hover:shadow-md transition-all group text-left">
            <span className={`p-2 rounded-lg ${a.tone}`}><a.icon size={14} /></span>
            <span className="text-[8px] font-black uppercase tracking-widest text-slate-700 dark:text-slate-300 group-hover:text-[#002D62] dark:group-hover:text-blue-400">{a.label}</span>
            <ChevronRight size={12} className="ml-auto text-slate-300 dark:text-slate-600 group-hover:text-[#002D62] dark:group-hover:text-blue-500 transition-colors" />
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {statCards.map((stat) => (
          <div
            ...
            className="bg-white dark:bg-[#032457] rounded-xl p-4 border border-slate-100 dark:border-white/5 shadow-md flex items-center justify-between hover:scale-[1.02] hover:border-[#002D62] dark:hover:border-blue-500/30 transition-all cursor-pointer"
          >
            <div>
      ...
      {/* Financial Telemetry */}
      <div className="bg-white dark:bg-[#032457] rounded-2xl shadow-lg border border-slate-100 dark:border-white/5 p-5">
        <div className="flex items-center justify-between mb-6 border-b border-slate-50 dark:border-white/5 pb-3">
          <div className="flex items-center gap-3">
            <DollarSign size={14} className="text-emerald-500" />
            <h3 className="text-[9px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Financial Telemetry</h3>
          </div>
          <button onClick={() => navigate('/admin/billing')} className="text-[7px] font-black text-[#002D62] dark:text-blue-400 uppercase tracking-widest hover:underline">Access Hub</button>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Total Burn", val: budgetOverview?.expenses, color: "text-slate-900 dark:text-white" },
            { label: "Spent Node", val: budgetOverview?.spent, color: "text-[#002D62] dark:text-blue-400" },
            { label: "Remaining", val: budgetOverview?.remaining, color: "text-blue-600 dark:text-blue-400" },
            { label: "Forecast", val: budgetOverview?.forecast, color: "text-violet-600 dark:text-violet-400" }
          ].map(node => (
            <div
              key={node.label}
              onClick={() => navigate('/admin/billing')}
              className="bg-slate-50 dark:bg-white/5 rounded-xl p-3 border border-slate-100 dark:border-white/5 cursor-pointer hover:bg-white dark:hover:bg-white/10 hover:shadow-inner transition-all"
            >
      ...
      <div className="bg-[#00122B] rounded-2xl shadow-2xl p-5 border border-white/5">
        <div className="flex items-center justify-between mb-5 border-b border-white/5 pb-3">
          <div className="flex items-center gap-3">
            <UserCheck size={14} className="text-[#002D62] dark:text-blue-400" />
            <h3 className="text-[9px] font-black text-white uppercase tracking-widest">Identity &amp; Trust</h3>
          </div>
          <button onClick={() => navigate('/admin/users')} className="text-[7px] font-black text-[#002D62] dark:text-blue-400 uppercase tracking-widest hover:text-white transition-colors">Manage Users</button>
        </div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative w-14 h-14 shrink-0">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                ...
                <circle cx="50" cy="50" r="40" fill="transparent" stroke="currentColor" strokeWidth="10" strokeDasharray="251.2" strokeDashoffset={251.2 - (251.2 * verifiedPercentage) / 100} strokeLinecap="round" className="text-[#002D62] dark:text-blue-400 transition-all duration-700" />
              </svg>
      ...
      {/* Latest Invoices */}
      <div className="bg-white dark:bg-[#032457] rounded-2xl shadow-lg p-5 border border-slate-100 dark:border-white/5">
        <div className="flex items-center justify-between mb-5 border-b border-slate-50 dark:border-white/5 pb-3">
          <div className="flex items-center gap-3">
            <DollarSign size={14} className="text-emerald-500" />
            <h3 className="text-[9px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Latest Invoices</h3>
          </div>
          <button onClick={() => navigate('/admin/billing')} className="text-[7px] font-black text-[#002D62] dark:text-blue-400 uppercase tracking-widest hover:underline">Billing Hub</button>
        </div>
        {recentInvoices.length > 0 ? (
          <div className="space-y-2">
            {recentInvoices.map((inv) => {
              const st = String(inv.status || 'draft').toLowerCase();
              const tone = st === 'paid' ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 border-emerald-100 dark:border-emerald-500/20'
                : st === 'overdue' ? 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 border-rose-100 dark:border-rose-500/20'
              ...
              return (
                <div key={inv.id} onClick={() => navigate(`/admin/billing/preview/${inv.id}`)}
                  className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-100 dark:border-white/5 hover:bg-white dark:hover:bg-white/10 cursor-pointer transition-all group">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="p-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 text-emerald-500"><DollarSign size={12} /></div>
                    <div className="min-w-0">
                      <p className="text-[9px] font-black text-slate-900 dark:text-white uppercase truncate">{inv.project_name || inv.client_name || `Invoice #${inv.id}`}</p>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <p className="text-[10px] font-black text-slate-900 dark:text-white">{formatKSH(inv.amount || inv.total_amount)}</p>
                    <ChevronRight size={11} className="text-slate-300 group-hover:text-[#002D62] dark:group-hover:text-blue-500" />
                  </div>
                </div>
              );
            })}
          ...
          <div className="py-8 text-center opacity-30">
            <FileCheck size={22} className="mx-auto mb-2 dark:text-white" />
            <p className="text-[7px] font-black uppercase dark:text-white">No invoices issued yet</p>
          </div>
        )}
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="bg-[#00122B] rounded-2xl shadow-2xl p-5 border border-white/5 relative overflow-hidden">
          <div className="flex items-center justify-between mb-6 border-b border-white/5 pb-3">
            <div className="flex items-center gap-3">
              <Activity size={14} className="text-[#002D62] dark:text-blue-400" />
              <h3 className="text-[9px] font-black text-white uppercase tracking-widest">Live Activity Log</h3>
            </div>
            <button
              onClick={() => navigate('/admin/activity')}
              className="text-[7px] font-black text-[#002D62] dark:text-blue-400 uppercase tracking-widest hover:text-white transition-colors"
            >
              Access Logs
            </button>
          </div>
          <div className="space-y-2">
            {recentActivity.length > 0 ? recentActivity.slice(0, 5).map((act, i) => (
              ...
              >
                <div className="w-6 h-6 rounded-full bg-white/5 flex items-center justify-center flex-shrink-0"><Clock size={10} className="text-slate-500 group-hover:text-[#002D62] dark:group-hover:text-blue-400" /></div>
            ...
            )) : <div className="py-10 text-center opacity-20"><Activity size={24} className="mx-auto mb-2 text-white" /><p className="text-[7px] font-black uppercase text-white">Silent Mode</p></div>}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#032457] rounded-2xl shadow-lg p-5 border border-slate-100 dark:border-white/5">
           <div className="flex items-center justify-between mb-6 border-b border-slate-50 dark:border-white/5 pb-3">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-[9px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Pending Validations</h3>
            </div>
          ...
          <div className="space-y-2">
            {pendingApprovals.length > 0 ? pendingApprovals.slice(0, 4).map((app, i) => (
              <div key={i} onClick={() => navigate('/admin/projects')} className="p-3 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-100 dark:border-white/5 flex items-center justify-between group hover:bg-white dark:hover:bg-white/10 transition-all cursor-pointer">
                <div><p className="text-[9px] font-black text-slate-900 dark:text-white uppercase">{app.name}</p><p className="text-[6px] text-slate-400 font-black uppercase tracking-widest mt-0.5">Awaiting Audit</p></div>
                <ChevronRight size={12} className="text-slate-300 dark:text-slate-600 group-hover:text-[#002D62] dark:group-hover:text-blue-500" />
              </div>
            )) : <div className="py-10 text-center opacity-20"><CheckCircle size={24} className="mx-auto mb-2 dark:text-white" /><p className="text-[7px] font-black uppercase dark:text-white">Nodes Clear</p></div>}
          </div>
        </div>
      </div>
