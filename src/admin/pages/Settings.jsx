...
function Toggle({ on, onClick, disabled }) {
  return (
    <button
      ...
      disabled={disabled}
      aria-pressed={on}
      className={`w-10 h-5 rounded-full relative transition-colors duration-200 shrink-0 ${on ? "bg-[#002D62]" : "bg-slate-300"} ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
    >
    ...
    <div className="space-y-6 animate-fade-in font-sans w-full">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => load()} disabled={loading} className="flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-white border border-slate-200 text-slate-600 font-black text-[8px] uppercase tracking-widest shadow-sm hover:border-blue-500 hover:text-blue-800 transition-all disabled:opacity-50">
            <RefreshCw size={12} /> Refresh
          </button>
          <button onClick={commit} disabled={saving || loading} className="flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-blue-700 text-white font-black text-[8px] uppercase tracking-widest shadow-md hover:bg-blue-800 transition-all disabled:opacity-50">
      ...
      {feedback && (
        <div className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-[9px] font-black uppercase tracking-widest ${feedback.type === "ok" ? "bg-blue-50 border-blue-200 text-blue-800" : "bg-rose-50 border-rose-200 text-rose-700"}`}>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <section className="bg-white rounded-2xl p-5 border border-slate-100 shadow-xl">
           ...
           <div className="space-y-3">
              ...
              <div>
                 ...
                 <input type="text" value={form.site_title || ""} onChange={(e) => set("site_title", e.target.value)} placeholder="The Greggory Systems And Strategy Firm" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-[9px] font-bold outline-none focus:ring-1 focus:ring-blue-500" />
              </div>
              <div>
                 <label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Public Contact Email</label>
                 <input type="email" value={form.contact_email || ""} onChange={(e) => set("contact_email", e.target.value)} placeholder="contact@greggoryfirm.com" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-[9px] font-bold outline-none focus:ring-1 focus:ring-blue-500" />
              </div>
              <div>
                 <label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Public Contact Phone</label>
                 <input type="text" value={form.contact_phone || ""} onChange={(e) => set("contact_phone", e.target.value)} placeholder="+254 …" className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-[9px] font-bold outline-none focus:ring-1 focus:ring-blue-500" />
              </div>

              {/* Theme Preference */}
              <div className="pt-2">
                 <label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Interface Theme</label>
                 <div className="flex items-center justify-between gap-2 p-2 bg-slate-50 rounded-lg border border-slate-100">
                    <div className="flex items-center gap-1.5">
                       {darkMode ? <Moon size={11} className="text-[#002D62]" /> : <Sun size={11} className="text-amber-500" />}
                       <p className="text-[8px] font-black text-slate-600 uppercase">{darkMode ? 'Dark Mode Active' : 'Light Mode Active'}</p>
                    </div>
               ...
               <div className="pt-2">
                  ...
                  <div className="flex items-center gap-3">
                     ...
                     <div className="flex-1">
                        <label className="flex items-center justify-center gap-1.5 px-3 py-2 bg-white border border-slate-200 rounded-lg text-[7px] font-black text-slate-600 uppercase tracking-widest cursor-pointer hover:border-blue-500 hover:text-blue-800 transition-all">
                           <Upload size={10} />
           ...
           <div className="space-y-2">
              ...
              {timeoutOn && (
                 <div className="flex items-center justify-between gap-2 p-2 bg-white rounded-lg border border-slate-100">
                    ...
                    <input type="number" min="10" max="720" step="5" value={form.admin_session_timeout || ""} onChange={(e) => set("admin_session_timeout", e.target.value)} className="w-16 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1 text-[9px] font-black text-center outline-none focus:ring-1 focus:ring-blue-500" />
                 </div>
              )}
              <div className="flex items-center justify-between gap-2 p-2 bg-slate-50 rounded-lg border border-slate-100">
                 <div className="flex items-center gap-1.5">
        ...
        <section className="bg-[#00122B] rounded-2xl p-5 border border-white/5 shadow-2xl flex flex-col">
           <div className="flex items-center gap-2 text-blue-400 border-b border-white/5 pb-3 mb-4">
      ...
      <section className="bg-white rounded-2xl p-5 border border-slate-100 shadow-xl">
         <div className="flex items-center justify-between border-b border-slate-50 pb-3 mb-4 flex-wrap gap-2">
            ...
            {calibration && (
               <span className={`px-2.5 py-1 rounded-full text-[7px] font-black uppercase tracking-widest ${calibration.status === "healthy" ? "bg-blue-100 text-blue-800" : "bg-amber-100 text-amber-700"}`}>
