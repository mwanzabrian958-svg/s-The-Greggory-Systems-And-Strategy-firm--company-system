...
export function PersonnelPreview() {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-[#00122B] flex items-center justify-center"><RefreshCw className="animate-spin text-blue-500" size={24} /></div>
    );
  }

  if (!person) {
    return (
      <div className="fixed inset-0 bg-[#00122B] flex flex-col items-center justify-center text-white p-6">
    ...
    <div className="fixed inset-0 bg-[#00122B] z-[500] flex flex-col overflow-hidden font-sans">
      <div className="bg-[#00122B] px-6 py-3 flex items-center justify-between border-b border-white/5 flex-shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-blue-500/10 rounded-lg flex items-center justify-center text-blue-500 border border-blue-500/20"><Briefcase size={16} /></div>
          <div className="hidden sm:block">
            <h2 className="text-lg font-black text-white uppercase leading-none tracking-tighter">Personnel Record Review</h2>
            <p className="text-[6px] text-blue-500 font-black uppercase tracking-[0.4em] mt-1">Company Personnel Relay View</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto p-8 lg:p-12">
          <div className="flex flex-col sm:flex-row gap-8 items-start">
            <div className="w-44 h-52 md:w-56 md:h-64 rounded-[28px] overflow-hidden shadow-2xl bg-white/2 border border-white/10 flex-shrink-0">
              {person.image_url ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-blue-500/10 text-6xl font-black text-blue-500">{person.name?.charAt(0) || "P"}</div>
              )}
            </div>
            <div className="pt-2 sm:pt-8 space-y-2">
              <span className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-400">Firm Personnel Profile</span>
              <h1 className="text-4xl md:text-5xl font-black text-white tracking-tighter leading-none uppercase">{person.name}</h1>
              <p className="text-sm font-bold text-blue-500 uppercase tracking-[0.2em]">{person.position}</p>
            </div>
          </div>

          <div className="h-px w-full bg-gradient-to-r from-blue-500/40 via-white/10 to-transparent my-10" />
          ...
          <div className="pt-10 flex flex-col items-center">
            <div className="w-px h-8 bg-gradient-to-b from-blue-500/50 to-transparent mb-3" />
            <p className="text-[7px] font-black text-slate-600 uppercase tracking-[0.8em]">End of Record</p>
          </div>
        </div>
      </div>

      <footer className="bg-[#00122B] border-t border-white/5 p-6 flex justify-between items-center flex-shrink-0">
