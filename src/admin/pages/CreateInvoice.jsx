...
export function CreateInvoice() {
  ...
  return (
    <div className="fixed inset-0 bg-[#00122B]/95 backdrop-blur-md z-[500] flex flex-col items-center justify-center p-4 font-sans overflow-hidden">
      <div className="bg-[#00122B] w-full max-w-4xl rounded-2xl border border-white/10 shadow-2xl flex flex-col max-h-[90vh]">
        <div className="px-6 py-3 flex items-center justify-between border-b border-white/5 bg-[#1e293b]/20">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center text-white font-black text-sm">GS</div>
            <div className="hidden sm:block">
              <h2 className="text-sm font-black text-white uppercase tracking-tight">Mission Deployment</h2>
              <p className="text-[6px] text-blue-500 font-black uppercase tracking-[0.4em]">Operational Protocol 7-Beta</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <section className="space-y-3">
                <div className="flex items-center gap-2 text-blue-500 border-b border-white/5 pb-1.5"><Briefcase size={10} /><h4 className="text-[7px] font-black uppercase tracking-widest">Alignment</h4></div>
                <div className="grid gap-3">
                  <div><label className="block text-[6px] font-black text-slate-500 uppercase tracking-widest mb-1 px-1">Project Node</label>
                    <select value={invoiceForm.project_id} onChange={(e) => setInvoiceForm({...invoiceForm, project_id: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-[9px] font-bold text-white outline-none focus:border-blue-500 transition-all" required>
                      <option value="" className="bg-[#00122B]">Select Mission...</option>
                      {projects.map(p => <option key={p.id} value={p.id} className="bg-[#00122B]">{p.project_name || p.name}</option>)}
                    </select>
                  </div>
                  <div><label className="block text-[6px] font-black text-slate-500 uppercase tracking-widest mb-1 px-1">Objective</label><input type="text" placeholder="Description..." value={invoiceForm.title} onChange={(e) => setInvoiceForm({...invoiceForm, title: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-[9px] font-bold text-white outline-none focus:border-blue-500 transition-all" required /></div>
                  <div><label className="block text-[6px] font-black text-slate-500 uppercase tracking-widest mb-1 px-1">Tax (KRA rate)</label>
                    <select value={invoiceForm.tax_rate} onChange={(e) => pickTaxPreset(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-[9px] font-bold text-white outline-none focus:border-blue-500 transition-all">
                      {TAX_PRESETS.map(p => <option key={p.type} value={p.pct} className="bg-[#00122B]">{p.label}</option>)}
                    </select>
                    <p className="mt-0.5 px-1 text-[6px] font-bold text-blue-400/80">{taxLabel(taxPct)} — {formatKSH(taxAmount)}</p>
                  </div>
                </div>
              </section>
            </div>
            ...
            <div className="md:col-span-2">
              <section className="space-y-3">
                <div className="flex items-center justify-between border-b border-white/5 pb-1.5">
                  ...
                  <button type="button" onClick={addLineItem} className="text-[7px] font-black text-blue-400 uppercase tracking-widest hover:text-white transition-colors">+ Add Item</button>
                </div>
                <div className="space-y-2">
                  ...
                  {invoiceForm.items.map((it, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                      <input type="text" placeholder="Service / deliverable..." value={it.description} onChange={(e) => updateItem(idx, 'description', e.target.value)} className="col-span-12 md:col-span-6 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-[9px] font-bold text-white outline-none focus:border-blue-500 transition-all" />
                      <input type="number" min="1" step="any" value={it.quantity} onChange={(e) => updateItem(idx, 'quantity', e.target.value)} className="col-span-4 md:col-span-2 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-[9px] font-bold text-white outline-none focus:border-blue-500 transition-all" />
                      <input type="number" min="0" step="any" placeholder="0" value={it.unit_price} onChange={(e) => updateItem(idx, 'unit_price', e.target.value)} className="col-span-5 md:col-span-2 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-[9px] font-bold text-white outline-none focus:border-blue-500 transition-all" />
        ...
        <div className="p-6 bg-[#00122B] border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
           <div className="flex gap-6">
              <div className="text-left">
                ...
                <p className="text-[5.5px] font-black text-blue-400 uppercase tracking-widest mt-1">{taxLabel(taxPct) || "No tax"}</p>
                <p className="text-[10px] font-bold text-blue-300">{formatKSH(taxAmount)}</p>
           ...
           <button onClick={handleInvoiceSubmit} disabled={isSubmitting} className="w-full md:w-auto bg-blue-700 hover:bg-blue-500 text-white px-8 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all shadow-xl flex items-center justify-center gap-2">
