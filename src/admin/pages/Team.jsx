      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-white border border-slate-200 rounded-lg overflow-hidden">
            <button onClick={() => setActiveTab("templates")} className={`px-3 py-1.5 text-[7px] font-black uppercase tracking-widest transition-all ${activeTab === "templates" ? "bg-blue-700 text-white" : "text-slate-500 hover:bg-slate-50"}`}>Crews</button>
            <button onClick={() => setActiveTab("members")} className={`px-3 py-1.5 text-[7px] font-black uppercase tracking-widest transition-all ${activeTab === "members" ? "bg-blue-700 text-white" : "text-slate-500 hover:bg-slate-50"}`}>Members</button>
          </div>
          {activeTab === "templates" ? (
            <button onClick={() => { setShowTemplateForm(true); setEditingTemplate(null); setTemplateForm({ name: '', description: '', project_id: '', team_leader_id: '', team_leader_image: null, team_leader_image_mime: '', member_ids: [] }); setTemplateImagePreview(null); }} className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-blue-700 text-white font-black text-[7px] uppercase tracking-widest shadow-md hover:bg-blue-800"><Plus size={12} /> New Crew</button>
          ) : (
            <button onClick={() => { setShowMemberForm(true); setEditingMember(null); setMemberForm({ name: '', role: '', department: '', description: '' }); }} className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-blue-700 text-white font-black text-[7px] uppercase tracking-widest shadow-md hover:bg-blue-800"><Plus size={12} /> Add Member</button>
          )}
        </div>
      </div>
          ...
          {showTemplateForm && (
            <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-lg">
              ...
              <form onSubmit={handleTemplateSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Crew / Group Name *</label><input type="text" value={templateForm.name} onChange={e => setTemplateForm({...templateForm, name: e.target.value})} placeholder="e.g. Alpha Design Squad" className="w-full px-3 py-2 border border-slate-200 rounded-lg text-[9px] font-bold outline-none focus:border-blue-500" required /></div>
                <div><label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Assigned Project</label><select value={templateForm.project_id} onChange={e => setTemplateForm({...templateForm, project_id: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-[9px] font-bold outline-none focus:border-blue-500"><option value="">— No project —</option>{projects.map(p => <option key={p.id} value={p.id}>{p.project_name}</option>)}</select></div>
                <div><label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Team Leader</label><select value={templateForm.team_leader_id} onChange={e => setTemplateForm({...templateForm, team_leader_id: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-[9px] font-bold outline-none focus:border-blue-500"><option value="">— Select leader —</option>{team.filter(m => m.is_active).map(m => <option key={m.id} value={m.id}>{m.name} ({m.role})</option>)}</select></div>
                <div><label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Leader Photo</label><div className="flex items-center gap-3">{templateImagePreview ? (<div className="relative"><img src={templateImagePreview} alt="Leader" className="w-12 h-12 rounded-xl object-cover border-2 border-slate-200" /><button type="button" onClick={() => { setTemplateImagePreview(null); setTemplateForm(f => ({ ...f, team_leader_image: null, team_leader_image_mime: '' })); }} className="absolute -top-1 -right-1 w-4 h-4 bg-rose-500 text-white rounded-full flex items-center justify-center"><X size={10} /></button></div>) : (<label className="flex items-center justify-center gap-1.5 px-3 py-2 bg-white border border-slate-200 rounded-lg text-[7px] font-black text-slate-600 uppercase tracking-widest cursor-pointer hover:border-blue-500 hover:text-blue-800 transition-all"><Upload size={10} /> Upload<input type="file" accept="image/*" onChange={handleTemplateImage} className="hidden" /></label>)}</div></div>
                <div className="md:col-span-2"><label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Description</label><textarea value={templateForm.description} onChange={e => setTemplateForm({...templateForm, description: e.target.value})} rows={2} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-[9px] font-bold outline-none focus:border-blue-500" /></div>
                <div className="md:col-span-2"><label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Crew Members ({templateForm.member_ids.length} selected)</label>{team.filter(m => m.is_active).length > 0 ? (<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-h-40 overflow-y-auto p-2 bg-slate-50 rounded-lg border border-slate-100">{team.filter(m => m.is_active).map(m => { const selected = templateForm.member_ids.includes(m.id); return (<button key={m.id} type="button" onClick={() => toggleMemberInTemplate(m.id)} className={`flex items-center gap-2 p-2 rounded-lg border text-left transition-all ${selected ? "bg-blue-50 border-blue-300 text-blue-900" : "bg-white border-slate-200 text-slate-600 hover:border-slate-300"}`}><div className="w-6 h-6 rounded-full bg-gradient-to-br from-blue-500 to-emerald-600 flex items-center justify-center text-white text-[7px] font-black shrink-0">{(m.name || '?').split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}</div><div className="min-w-0"><p className="text-[7px] font-black truncate">{m.name}</p><p className="text-[6px] truncate opacity-70">{m.role}</p></div></button>); })}</div>) : (<p className="text-[8px] text-slate-400 py-3 text-center">No active team members. Add members first.</p>)}</div>
                <div className="md:col-span-2 flex gap-3 pt-2"><button type="submit" className="px-4 py-2 bg-blue-700 text-white rounded-lg text-[7px] font-black uppercase tracking-widest hover:bg-blue-800">Save Crew</button><button type="button" onClick={() => { setShowTemplateForm(false); setEditingTemplate(null); setTemplateImagePreview(null); }} className="px-4 py-2 border border-slate-200 text-slate-600 rounded-lg text-[7px] font-black uppercase tracking-widest hover:bg-slate-50">Cancel</button></div>
              </form>
            </div>
          )}
          ...
          {showMemberForm && (
            <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-lg">
              ...
              <form onSubmit={handleMemberSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Full Name</label><input type="text" value={memberForm.name} onChange={e => setMemberForm({...memberForm, name: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-[9px] font-bold outline-none focus:border-blue-500" /></div>
                <div><label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Role / Title</label><input type="text" value={memberForm.role} onChange={e => setMemberForm({...memberForm, role: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-[9px] font-bold outline-none focus:border-blue-500" /></div>
                <div><label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Department</label><input type="text" value={memberForm.department} onChange={e => setMemberForm({...memberForm, department: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-[9px] font-bold outline-none focus:border-blue-500" /></div>
                <div><label className="block text-[6px] font-black text-slate-400 uppercase tracking-widest mb-1 px-1">Description</label><textarea value={memberForm.description} onChange={e => setMemberForm({...memberForm, description: e.target.value})} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-[9px] font-bold outline-none focus:border-blue-500" /></div>
                <div className="md:col-span-2 flex gap-3 pt-2"><button type="submit" className="px-4 py-2 bg-blue-700 text-white rounded-lg text-[7px] font-black uppercase tracking-widest hover:bg-blue-800">Save</button><button type="button" onClick={() => { setShowMemberForm(false); setEditingMember(null); }} className="px-4 py-2 border border-slate-200 text-slate-600 rounded-lg text-[7px] font-black uppercase tracking-widest hover:bg-slate-50">Cancel</button></div>
              </form>
            </div>
          )}
          <div className="bg-white rounded-xl p-4 border border-slate-100 shadow-md">
            <div className="flex items-center gap-3 mb-4"><Search size={14} className="text-slate-400" /><input type="text" placeholder="Search team members..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="flex-1 px-3 py-2 border border-slate-200 rounded-lg text-[9px] font-bold outline-none focus:border-blue-500" /></div>
            {filteredMembers.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredMembers.map((member) => (
                  <div key={member.id} className="bg-slate-50 rounded-xl p-4 border border-slate-100 hover:border-blue-500/30 transition-all">
                    <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-emerald-600 flex items-center justify-center text-white text-xs font-black">{(member.name || '?').split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}</div><div><p className="text-[9px] font-black text-slate-900">{member.name}</p><p className="text-[7px] text-slate-500 uppercase font-bold">{member.role}</p></div></div><div className="flex gap-1"><button onClick={() => handleMemberEdit(member)} className="p-1.5 hover:bg-blue-50 rounded-lg transition-colors"><Edit2 size={12} className="text-blue-700" /></button><button onClick={() => handleMemberDelete(member.id)} className="p-1.5 hover:bg-rose-50 rounded-lg transition-colors"><Trash2 size={12} className="text-rose-600" /></button></div></div>
