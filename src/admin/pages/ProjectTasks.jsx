...
export function ProjectTasks() {
  ...
  if (loading) return <div className="fixed inset-0 bg-[#00122B] flex items-center justify-center"><RefreshCw className="animate-spin text-blue-500" size={24} /></div>;

  return (
    <div className="fixed inset-0 bg-[#00122B] z-[500] flex flex-col overflow-hidden font-sans text-white">
      {/* Header */}
      <div className="bg-[#00122B] px-6 py-3 flex items-center justify-between border-b border-white/5 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:block">
            ...
            <p className="text-[7px] text-slate-500 font-black uppercase tracking-[0.4em] mt-1">Project Node: <span className="text-blue-400">{project?.project_name || "Unknown"}</span></p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-blue-700 text-white rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-blue-800 shadow-lg"
          >
                ...
                <div className="mt-auto space-y-4">
                   ...
                   <div className="grid grid-cols-2 gap-2">
                      <select
                        ...
                        className="bg-white/5 border border-white/10 rounded-lg py-2 px-3 text-[8px] font-black uppercase text-white outline-none focus:border-blue-500"
                      >
                         <option value="not_started" className="bg-[#00122B]">NOT STARTED</option>
                         <option value="in_progress" className="bg-[#00122B]">IN PROGRESS</option>
                         <option value="completed" className="bg-[#00122B]">COMPLETED</option>
                         <option value="blocked" className="bg-[#00122B]">BLOCKED</option>
                      </select>
      ...
      {showAddForm && (
        <div className="fixed inset-0 bg-[#00122B] z-[600] animate-in fade-in duration-300 flex items-center justify-center p-6">
           <div className="bg-slate-900 border border-white/10 rounded-[40px] shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
              ...
              <form onSubmit={handleAddTask} className="p-8 space-y-6 flex-1 overflow-y-auto">
                 <div>
                    ...
                    <input type="text" value={newTask.task_name} onChange={(e) => setNewTask({...newTask, task_name: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[11px] font-bold outline-none focus:border-blue-500" placeholder="Objective name..." required />
                 </div>
                 <div>
                 ...
                 <div className="grid grid-cols-2 gap-6">
                    <div>
                       ...
                       <select value={newTask.assigned_to} onChange={(e) => setNewTask({...newTask, assigned_to: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[9px] font-bold outline-none">
                          <option value="" className="bg-[#00122B]">Select Personnel...</option>
                          {team.map(t => <option key={`${t.id}`} value={t.id} className="bg-[#00122B]">{t.display_name}</option>)}
                       </select>
                    </div>
                    <div>
                 ...
                 <div className="pt-10 flex gap-4">
                    ...
                    <button type="submit" disabled={isSubmitting} className="flex-[2] bg-blue-700 hover:bg-blue-500 py-4 rounded-2xl text-[10px] font-black uppercase flex items-center justify-center gap-3">
