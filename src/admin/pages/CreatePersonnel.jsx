...
export function CreatePersonnel() {
  ...
  return (
    <div className="fixed inset-0 bg-[#00122B] z-[500] flex flex-col overflow-hidden font-sans">
      <div className="bg-[#00122B] px-6 py-3 flex items-center justify-between border-b border-white/5 flex-shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-blue-500/10 rounded-lg flex items-center justify-center text-blue-500 border border-blue-500/20"><Users size={16} /></div>
          <div className="hidden sm:block">
            <h2 className="text-lg font-black text-white uppercase leading-none tracking-tighter">Personnel Deployment</h2>
            <p className="text-[6px] text-blue-500 font-black uppercase tracking-[0.4em] mt-1">Company Personnel Relay</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
        <div className="w-full p-6 lg:p-10">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="space-y-6">
              <section className="space-y-4 bg-white/2 p-5 rounded-2xl border border-white/5">
                <div className="flex items-center gap-2 text-blue-500 border-b border-white/5 pb-2"><ImageIcon size={10} /><h4 className="text-[8px] font-black uppercase tracking-[0.2em]">Profile Image</h4></div>
            ...
            <div className="space-y-6 lg:col-span-2">
              <section className="space-y-4 bg-white/2 p-5 rounded-2xl border border-white/5">
                <div className="flex items-center gap-2 text-blue-500 border-b border-white/5 pb-2"><Type size={10} /><h4 className="text-[8px] font-black uppercase tracking-[0.2em]">Identity Node</h4></div>
                <div className="grid gap-4">
                  <div>
                    <label className="block text-[6px] font-black text-slate-500 uppercase tracking-widest mb-1 px-1">Full Name</label>
                    <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required placeholder="e.g. Brian Mwanza" className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-[9px] font-bold text-white outline-none focus:border-blue-500" />
                  </div>
                  <div>
                    <label className="block text-[6px] font-black text-slate-500 uppercase tracking-widest mb-1 px-1">Position</label>
                    <div className="relative"><Briefcase className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-600" size={10} /><input value={form.position} onChange={(e) => setForm({ ...form, position: e.target.value })} required placeholder="e.g. Founder & Managing Director" className="w-full bg-white/5 border border-white/10 rounded-lg pl-8 pr-3 py-2 text-[9px] font-bold text-white outline-none focus:border-blue-500" /></div>
                  </div>
                  <div>
                    ...
                    <textarea value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-[9px] font-medium text-slate-300 outline-none h-[320px] resize-none leading-relaxed focus:border-blue-500" placeholder="Full credentials, history and records posted about this person... HTML is supported. Plain text will be formatted into readable paragraphs." />
                  </div>
                </div>
              </section>
            </div>
          ...
          <div className="lg:col-span-3 bg-[#00122B] rounded-2xl p-6 border border-white/10 shadow-2xl flex flex-col md:flex-row justify-between items-center gap-6 mt-4">
            <div className="flex gap-6">
              <div className="bg-white/5 px-6 py-3 rounded-xl border border-white/10"><p className="text-[6px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Status</p><p className="text-lg font-black text-blue-400 leading-none">READY</p></div>
            ...
            <button type="submit" disabled={isSubmitting} className="bg-blue-700 hover:bg-blue-500 text-white px-10 py-3 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] shadow-xl flex items-center gap-3 border border-blue-400/20 transition-all active:scale-95">
