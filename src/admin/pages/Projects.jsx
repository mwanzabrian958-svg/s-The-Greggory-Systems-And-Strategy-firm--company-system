    ...
    <div className="space-y-6 animate-fade-in font-sans w-full">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <div className="bg-slate-100 p-1 rounded-lg flex gap-1">
             <button onClick={() => setViewMode("kanban")} className={`p-1.5 rounded-md transition-all ${viewMode === 'kanban' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-400'}`}><LayoutDashboard size={12} /></button>
             <button onClick={() => setViewMode("list")} className={`p-1.5 rounded-md transition-all ${viewMode === 'list' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-400'}`}><BarChart3 size={12} /></button>
          </div>
          <button onClick={() => setIsCreateModalOpen(true)} className="flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-blue-700 text-white font-black text-[8px] uppercase tracking-widest shadow-md hover:bg-blue-800 border border-blue-400/20">
      ...
      <div className="flex gap-4 items-center">
         ...
         <select
            ...
            className="bg-white border border-slate-100 rounded-xl px-4 py-2.5 text-[10px] font-black uppercase tracking-widest outline-none shadow-sm cursor-pointer hover:border-blue-500 transition-colors"
          >
      ...
      {focusedCategory && (
        <div className="fixed inset-0 bg-[#f8fafc] z-[1000] flex flex-col animate-in fade-in zoom-in-95 duration-300">
          {/* Fullscreen Navbar */}
          <div className="bg-[#00122B] text-white px-8 py-4 flex items-center justify-between border-b border-white/5 shadow-2xl">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[8px] text-blue-500 font-black uppercase tracking-[0.4em] mt-1">Full-Screen Strategic View</p>
              </div>
            </div>
                   ...
                   {filteredProjects.filter(p => (p.status || 'planning') === focusedCategory.id).length > 0 ? (
                     filteredProjects.filter(p => (p.status || 'planning') === focusedCategory.id).map(p => (
                      ...
                      >
                        ...
                        <div className="mb-8">
                          <h4 className="text-sm font-black text-slate-900 uppercase group-hover:text-blue-700 transition-colors mb-2">{p.project_name}</h4>
                          <div className="flex items-center gap-2 mb-4">
                            ...
                            <span className="text-[9px] text-blue-700 font-bold">{formatKSH(p.estimated_budget)}</span>
                          </div>
                        </div>

                        <div className="space-y-6">
                           <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                              <div className="h-full bg-blue-500 rounded-full transition-all duration-1000 shadow-[0_0_10px_rgba(20,184,166,0.3)]" style={{ width: `${p.progress_percentage || 0}%` }} />
                           </div>
                           <div className="flex justify-between items-center text-[8px] font-black uppercase text-slate-400 tracking-widest">
                              <span className="flex items-center gap-2"><TrendingUp size={12} className="text-blue-500" /> {p.progress_percentage || 0}% SYNC</span>
                              <button onClick={(e) => { e.stopPropagation(); navigate(`/admin/projects/${p.id}/tasks`); }} className="flex items-center gap-2 text-blue-700 hover:text-blue-800 transition-colors"><CheckSquare size={12} /> Access Tasks</button>
                           </div>
                        </div>
                      </div>
                     ))
