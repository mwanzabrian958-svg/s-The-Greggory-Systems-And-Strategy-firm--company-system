...
import { RefreshCw, Shield, ShieldCheck, ShieldAlert, Activity, Users, FileText, Database, Play } from "lucide-react";
...
export function DataSafety() {
  ...
  const getStatusIcon = (status) => {
    switch(status) {
      ...
      case 'failure': return <ShieldAlert size={12} className="text-rose-500" />;
  ...
  return (
    <div className="space-y-6 animate-fade-in font-sans w-full">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={backupBusy}
                className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg font-black text-[7px] uppercase tracking-widest text-white ${backupBusy ? "bg-slate-400 cursor-not-allowed" : "bg-blue-700 hover:bg-blue-800"}`}
              >
      ...
      <div className="flex gap-2">
        <button onClick={() => setActiveTab('summary')} className={`px-4 py-2 rounded-lg text-[7px] font-black uppercase tracking-widest ${activeTab === 'summary' ? 'bg-blue-700 text-white' : 'bg-white border border-slate-200 text-slate-700 hover:bg-slate-50'}`}>Summary</button>
        <button onClick={() => setActiveTab('audit')} className={`px-4 py-2 rounded-lg text-[7px] font-black uppercase tracking-widest ${activeTab === 'audit' ? 'bg-blue-700 text-white' : 'bg-white border border-slate-200 text-slate-700 hover:bg-slate-50'}`}>Audit Logs</button>
        <button onClick={() => setActiveTab('access')} className={`px-4 py-2 rounded-lg text-[7px] font-black uppercase tracking-widest ${activeTab === 'access' ? 'bg-blue-700 text-white' : 'bg-white border border-slate-200 text-slate-700 hover:bg-slate-50'}`}>Data Access</button>
        <button onClick={() => setActiveTab('backup')} className={`px-4 py-2 rounded-lg text-[7px] font-black uppercase tracking-widest ${activeTab === 'backup' ? 'bg-blue-700 text-white' : 'bg-white border border-slate-200 text-slate-700 hover:bg-slate-50'}`}>Backup</button>
      </div>
    </div>
  );
}
