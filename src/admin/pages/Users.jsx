    ...
    <div className="space-y-6 animate-fade-in font-sans w-full">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => navigate('/admin/users/manage')} className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-blue-700 text-white font-black text-[7px] uppercase tracking-widest shadow-md hover:bg-blue-800 border border-blue-400/20">
      ...
      <div className="bg-[#00122B] rounded-xl p-4 border border-white/10 shadow-xl flex justify-between items-center">
         <div className="flex items-center gap-3 text-blue-400"><Filter size={14} /><span className="text-[8px] font-black uppercase tracking-widest">Filters</span></div>
         <div className="flex gap-3">
            <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-[8px] font-black uppercase text-white outline-none">
              <option value="all" className="bg-[#00122B]">All Roles</option>
              <option value="admin" className="bg-[#00122B]">Admins</option>
              <option value="client" className="bg-[#00122B]">Clients</option>
            </select>
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-[8px] font-black uppercase text-white outline-none">
              <option value="all" className="bg-[#00122B]">All Status</option>
              <option value="active" className="bg-[#00122B]">Active</option>
              <option value="inactive" className="bg-[#00122B]">Inactive</option>
            </select>
         </div>
      </div>
            ...
            <div key={`${u.source_table}-${u.id}`} className="bg-white rounded-2xl p-4 border border-slate-100 shadow-md hover:scale-[1.03] transition-all group flex flex-col cursor-pointer" onClick={() => navigateToDetail(u)}>
              <div className="flex items-center gap-3 mb-4">
                <div className="relative">
                  <div className="w-10 h-10 rounded-lg bg-[#00122B] flex items-center justify-center text-white font-black text-sm">{(u.display_name || u.name || "U")[0]}</div>
                ...
                <div className="min-w-0 flex-1">
                  <h4 className="font-black text-slate-900 text-[10px] uppercase truncate group-hover:text-blue-700">{u.display_name || u.name}</h4>
                  <p className="text-[7px] text-slate-400 font-bold uppercase truncate">{u.email}</p>
                  {u.phone_number && <p className="text-[6.5px] text-blue-700 font-black uppercase mt-0.5 tracking-tighter">{u.phone_number}</p>}
                </div>
              </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-6 pb-12">
          ...
          <div className="px-4 py-2 bg-[#00122B] rounded-xl text-[8px] font-black text-white uppercase tracking-widest border border-white/10">Page {currentPage} / {totalPages}</div>
