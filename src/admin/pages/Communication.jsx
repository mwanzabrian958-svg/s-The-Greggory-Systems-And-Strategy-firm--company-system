...
export function Communication() {
  ...
  if (loading && clients.length === 0) return <div className="flex items-center justify-center py-20"><RefreshCw className="animate-spin text-blue-700 w-6 h-6" /></div>;
          ...
          {["messages", "announcements", "broadcast"].map((tab) => (
            <button
              ...
              className={`flex-1 px-4 py-3 text-[8px] font-black uppercase tracking-widest transition-all ${
                activeTab === tab ? "bg-white border-b-2 border-blue-500 text-blue-700" : "text-slate-400 hover:text-slate-600"
              }`}
            >
              {tab}
            </button>
            ...
            <form onSubmit={handleSendFeedback} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={selectedClientId} onChange={(e) => setSelectedClientId(e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-xl px-3 py-2.5 text-[9px] font-bold outline-none focus:ring-1 focus:ring-blue-500">
              ...
              <div>
                ...
                <input type="text" value={feedbackTitle} onChange={(e) => setFeedbackTitle(e.target.value)} placeholder="Node identification..." className="w-full bg-slate-50 border border-slate-100 rounded-xl px-3 py-2.5 text-[9px] font-bold outline-none focus:ring-1 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-[7px] font-black text-slate-500 uppercase tracking-widest mb-1.5 px-1">Manuscript Body</label>
                <textarea rows="4" value={feedbackMessage} onChange={(e) => setFeedbackMessage(e.target.value)} placeholder="Enter technical update for portal relay..." className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-[9px] font-medium text-slate-600 outline-none focus:ring-1 focus:ring-blue-500 resize-none leading-relaxed" />
              </div>
              ...
              <button type="submit" disabled={isSending} className="w-full bg-blue-700 text-white py-3 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-blue-800 transition-all shadow-xl flex items-center justify-center gap-3 disabled:opacity-50">
