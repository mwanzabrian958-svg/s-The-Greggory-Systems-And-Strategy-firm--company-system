...
export function SearchResults() {
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'user': return <User className="text-blue-400" size={20} />;
  ...
  return (
    <div className="space-y-8 animate-fade-in font-sans w-full pb-20">
      {/* Header telemetry */}
      <div className="flex items-end justify-between border-b border-slate-200 pb-6">
        <div>
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.4em] text-[8px] mt-2">
            Parameter: <span className="text-blue-700">"{query}"</span> • Results: <span className="text-slate-900">{results.length} Nodes</span>
          </p>
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-40">
           <RefreshCw className="animate-spin text-blue-700 w-10 h-10 mb-4" />
           <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Scanning System Backbone...</p>
        </div>
      ) : results.length > 0 ? (
        <div className="grid gap-6">
          {results.map((item, idx) => (
            <div key={`${item.type}-${item.id}-${idx}`} className="bg-white rounded-[32px] border border-slate-100 shadow-xl overflow-hidden hover:border-blue-500/30 transition-all group">
              <div className="flex flex-col md:flex-row">
                ...
                <div className="md:w-64 bg-slate-50 p-8 flex flex-col items-center justify-center border-r border-slate-100">
                   <div className="w-20 h-20 rounded-3xl bg-[#00122B] flex items-center justify-center text-white mb-4 shadow-2xl">
                      {getIcon(item.type)}
                   </div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                         ...
                         <div className="space-y-3">
                            ...
                            {item.role && (
                               <div className="flex items-center gap-3">
                                  ...
                                  <span className="text-[10px] font-black uppercase text-blue-700">{item.role}</span>
                               </div>
                            )}
                         </div>
                      </div>
                      ...
                      <div className="flex flex-col justify-between">
                         ...
                         <button
                            ...
                            className="mt-6 w-full py-3 bg-[#00122B] text-white rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-black transition-all"
                         >
                            Initialize Detail View <ArrowRight size={14} className="text-blue-400" />
                         </button>
                      </div>
                   </div>
                </div>
