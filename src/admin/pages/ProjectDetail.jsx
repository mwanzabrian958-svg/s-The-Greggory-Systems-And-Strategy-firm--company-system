...
export function ProjectDetail() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 w-full">
      <Link to="/admin/projects" className="inline-flex items-center gap-2 text-[8px] font-black text-slate-500 uppercase tracking-widest hover:text-blue-700">
        ...
        {["tasks", "files", "timeline"].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-4 py-2 text-[8px] font-black uppercase tracking-widest border-b-2 transition-all ${
              activeTab === tab ? "border-blue-700 text-blue-700" : "border-transparent text-slate-400 hover:text-slate-600"
            }`}>{tab}</button>
        ))}
      </div>
      {activeTab === "tasks" && (
