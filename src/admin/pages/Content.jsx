      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
           <div className="bg-slate-100 p-1 rounded-xl flex gap-1">
              <button onClick={() => setActiveTab("blogs")} className={`px-4 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${activeTab === 'blogs' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-400'}`}>Manuscripts</button>
              <button onClick={() => setActiveTab("website")} className={`px-4 py-1.5 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${activeTab === 'website' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-400'}`}>Site Elements</button>
           </div>
           {activeTab === "blogs" && (
             <button onClick={() => navigate("/admin/content/create")} className="flex items-center gap-2 px-4 py-1.5 rounded-xl bg-blue-700 text-white font-black text-[8px] uppercase tracking-widest shadow-lg hover:bg-blue-800 transition-all border border-blue-400/20"><Plus size={12} /> New Post</button>
           )}
        </div>
      </div>
      ...
      {activeTab === "blogs" ? (
        <>
          <div className="bg-[#00122B] rounded-2xl p-4 border border-white/10 shadow-xl flex justify-between items-center">
            <div className="text-[7px] font-black uppercase text-blue-400 tracking-[0.4em] px-2">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12"><RefreshCw className="animate-spin text-blue-700 w-6 h-6" /></div>
          ) : (
              ...
              {paginatedBlogs.map((item) => (
                ...
                >
                  <div className="aspect-[4/3] bg-slate-100 rounded-xl mb-3 overflow-hidden border border-slate-50 relative">
                     ...
                     <div className="absolute top-1.5 left-1.5 bg-[#00122B]/80 backdrop-blur-sm text-white px-1.5 py-0.5 rounded-md text-[5px] font-black uppercase tracking-widest">{item.category || "General"}</div>
                  </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-6 pb-12">
              ...
              <div className="px-4 py-2 bg-[#00122B] rounded-xl text-[8px] font-black text-white uppercase tracking-widest border border-white/10 shadow-lg">Node {currentPage} / {totalPages}</div>
        ...
        <div className="grid gap-6">
           <div className="bg-[#00122B] rounded-3xl p-8 border border-white/10 shadow-2xl">
              <div className="flex items-center gap-3 mb-8 text-blue-400 border-b border-white/5 pb-4"><FileText size={18} /><h4 className="text-[11px] font-black uppercase tracking-widest">Master site elements</h4></div>
              <div className="grid gap-6">
                 {siteContent.map(item => (
                   <div key={item.content_key} className="bg-white/2 border border-white/5 rounded-2xl p-6 group hover:bg-white/5 transition-all">
                      <div className="flex justify-between items-start mb-4">
                         ...
                         <span className="text-[6px] font-black px-2 py-0.5 bg-blue-500/10 text-blue-500 rounded border border-blue-500/20 uppercase">{item.content_type}</span>
                      </div>
                      <textarea
                        ...
                        className="w-full bg-slate-950 border border-white/5 rounded-xl px-4 py-3 text-[10px] font-medium text-slate-300 outline-none focus:border-blue-500/50 min-h-[80px] resize-none"
                      />
                   </div>
                 ))}
              </div>
              {isUpdating && (
                <div className="fixed bottom-10 right-10 bg-blue-700 text-white px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-3 animate-bounce">
