...
export function Personnel() {
  ...
  return (
    <div className="space-y-6 animate-fade-in font-sans">
        ...
        <div className="flex gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="pl-7 pr-3 py-1.5 bg-white border border-slate-100 rounded-xl text-[8px] font-bold text-slate-600 outline-none focus:border-blue-500/50 w-44"
            />
          </div>
          <button
            onClick={() => navigate("/admin/personnel/create")}
            className="px-4 py-2 bg-blue-700 hover:bg-blue-500 text-white rounded-xl text-[8px] font-black uppercase tracking-widest shadow-lg flex items-center gap-2 transition-all"
          >
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20"><RefreshCw className="animate-spin text-blue-500" size={20} /></div>
      ) : paginated.length === 0 ? (
        <div className="bg-white rounded-3xl border border-slate-100 p-12 text-center">
          ...
          <button onClick={() => navigate("/admin/personnel/create")} className="mt-4 px-4 py-2 bg-blue-700 text-white rounded-xl text-[8px] font-black uppercase tracking-widest">Create the first profile</button>
        </div>
      ) : (
        <>
              ...
              >
                <div className="relative aspect-[4/5] bg-slate-50 overflow-hidden">
                  {item.image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-blue-50 text-4xl font-black text-blue-700">{item.name?.charAt(0) || "P"}</div>
                  )}
                </div>
                <div className="p-4">
                  <p className="text-[11px] font-black text-slate-900 truncate">{item.name}</p>
                  <div className="flex items-center justify-between mt-2">
                    <p className="text-[7px] font-black uppercase tracking-widest text-blue-700 truncate">{item.position}</p>
