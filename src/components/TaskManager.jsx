      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center space-x-4 flex-1">
          <button
            ...
            className={`w-6 h-6 rounded-xl border-2 flex items-center justify-center transition-all ${task.status === 'completed' ? 'bg-emerald-500 border-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'border-white/20 hover:border-blue-500'}`}
          >
...
const TaskForm = ({ task, onSave, onCancel, isEditing }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white/5 backdrop-blur-3xl rounded-[32px] p-8 border border-white/10 shadow-2xl mb-10 animate-fade-in">
      <h3 className="text-sm font-black text-blue-500 uppercase tracking-[0.4em] mb-8">{isEditing ? 'Modify Protocol' : 'Initialize New Protocol'}</h3>
      <div className="space-y-6">
        <div>
          ...
          <input
            ...
            className="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-white font-black text-xs uppercase tracking-widest focus:outline-none focus:ring-2 focus:ring-blue-500/40 transition-all"
            required
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            className="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-white font-medium text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 transition-all resize-none"
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <select
              ...
              className="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-white font-black text-[10px] uppercase tracking-widest focus:outline-none focus:ring-2 focus:ring-blue-500/40 transition-all"
            >
        ...
        <div className="flex justify-end gap-4 pt-6 border-t border-white/5">
          ...
          <button
            type="submit"
            className="px-10 py-4 text-[10px] font-black text-white uppercase tracking-widest bg-blue-700 rounded-2xl hover:bg-blue-600 transition-all shadow-xl shadow-blue-700/30"
          >
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-6 mb-12">
        <div>
           <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] mb-2 text-center sm:text-left">Module Control</p>
        ...
        <button
          ...
          className="flex items-center gap-3 px-8 py-4 bg-blue-700 text-white rounded-[20px] font-black text-[10px] uppercase tracking-[0.2em] hover:bg-blue-600 transition-all shadow-xl shadow-blue-700/30"
        >
          <Plus size={18} />
        ...
        {['all', 'pending', 'in_progress', 'completed'].map((status) => (
          <button
            ...
            className={`px-6 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all border ${
              filter === status
                ? 'bg-blue-700 text-white border-blue-700 shadow-lg shadow-blue-700/30'
