...
export default function AuthPlatformModal({
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      // Upload the profile photo first (same relay the admin login page uses),
      // then reference it by id — the register endpoint expects profile_image_id.
      let profile_image_id = null;
      if (profilePhoto && profilePhotoPreview) {
        try {
          const base64 = String(profilePhotoPreview).split(",")[1];
          if (base64) {
            ...
            profile_image_id = imgRes.image_id || null;
          }
        } catch (photoErr) {
          console.error("[AUTH PHOTO UPLOAD] failed, continuing without photo:", photoErr);
        }
      }
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("gf_admin_user", JSON.stringify({ ...data.user, role: "admin", token: data.token }));
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/55 backdrop-blur-sm">
      <div className="relative w-full max-w-lg max-h-[92vh] overflow-hidden rounded-2xl shadow-2xl border border-white/15 bg-slate-900 flex flex-col font-sans">
        ...
        <div className="overflow-y-auto flex-1">
          {view === "platform" && (
            <section className="bg-gradient-to-r from-[#00122B] via-[#002D62] to-[#00122B] text-white border-b border-blue-400/30 px-6 pt-10 pb-8 sm:px-8">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-[#3B82F6] text-white text-[10px] font-black uppercase tracking-widest hover:bg-[#2563EB] transition-colors sm:ml-auto"
                >
            ...
            <div className="bg-white px-6 py-8 sm:px-8 relative">
              ...
              <div className="text-center mb-6">
                <div className="relative w-20 h-20 mb-4 shrink-0 mx-auto">
                  <div className="w-full h-full rounded-full bg-gradient-to-r from-[#002D62] to-[#3B82F6] overflow-hidden flex items-center justify-center border-2 border-slate-100 shadow-lg">
                    {profilePhotoPreview ? (
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                ...
                <button type="submit" disabled={loginLoading} className="w-full px-4 py-3 bg-gradient-to-r from-[#002D62] to-[#3B82F6] text-white rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-[#002D62]/20">
              ...
              <div className="mt-6 pt-6 border-t border-gray-100">
                ...
                <button onClick={handleRegisterStart} className="w-full px-4 py-2 border border-blue-200 text-[#002D62] rounded-lg text-[8px] font-black uppercase tracking-widest hover:bg-blue-50 transition-all">Register Admin Node</button>
              </div>
            </div>
          )}
          ...
          {view === "register" && (
            <div className="bg-white px-6 py-8 sm:px-8 relative">
              ...
              <div className="text-center mb-6">
                <div className="relative w-20 h-20 mb-4 shrink-0 mx-auto">
                  <div className="w-full h-full rounded-full bg-gradient-to-r from-[#002D62] to-[#3B82F6] flex items-center justify-center overflow-hidden border-2 border-slate-100 shadow-lg">
              ...
              {regStep === 3 ? (
                <div className="text-center py-8">
                  ...
                  <button onClick={() => setView("admin")} className="w-full px-4 py-3 bg-[#D4AF37] hover:bg-[#B8860B] text-black rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl">Proceed to Verification</button>
                </div>
              ) : (
                <form onSubmit={handleRegisterSubmit} className="space-y-4">
                  ...
                  <button type="submit" disabled={isSubmitting} className="w-full px-4 py-3 bg-gradient-to-r from-[#002D62] to-[#3B82F6] text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">{isSubmitting ? "Solidifying..." : "Initialize Node"}</button>
                  <p className="text-center text-[7px] font-black text-gray-400 uppercase tracking-widest">Pre-existing Node? <button type="button" onClick={() => setView("admin")} className="text-[#002D62]">Access Console here</button></p>
                </form>
              )}
            </div>
          )}
