...
const BrandHeader = ({
  ...
  return (
    <div className={`inline-block bg-white/90 shadow-md ${s.border} border-[#002D62]/20 rounded-lg ${s.padding} ${isCenter ? 'mx-auto' : ''} ${wrapperClass}`}>
      <div className={`flex ${isCenter ? 'flex-col items-center text-center' : 'flex-row items-start'}`}>
        ...
        <div className={`flex flex-col ${isCenter ? 'items-center text-center' : 'items-start'}`}>
          <h1 className={`${s.title} font-extrabold text-[#002D62]`}>
            {wordmark}
          </h1>
          {tagline && (
            <p className={`${s.tagline} font-medium text-[#3B82F6] mt-1`}>
              {tagline}
            </p>
          )}
        </div>
