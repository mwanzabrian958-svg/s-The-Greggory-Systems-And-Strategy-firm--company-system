    ...
    try {
        ...
        if (users.length > 0) {
            ...
            if (await bcryptjs.compare(password, user.password_hash)) {
                ...
                await mainDb.query('UPDATE admin_users SET last_login_at = NOW(), last_login_ip = ? WHERE id = ?', [req.ip || 'unknown', user.id]);
                return res.json({
                    success: true, token,
                    user: { id: user.id, email: user.email, name: user.display_name || `${user.first_name} ${user.last_name}`, role: "admin", admin_level: user.admin_level || "admin", photoData: getProfilePhotoData(user) }
                });
            }
        }
...
app.post("/api/admin-verification/register", async (req, res) => {
    ...
    try {
        ...
        if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ success: false, message: "Identity Conflict" });
...
app.get("/api/invoices", async (req, res) => {
    try {
        const [rows] = await mainDb.query("SELECT * FROM invoices WHERE deleted_at IS NULL ORDER BY created_at DESC");
...
app.get("/api/admin/ledger", async (req, res) => {
    try {
        const [rows] = await mainDb.query("SELECT * FROM accounting_entries WHERE deleted_at IS NULL ORDER BY transaction_date DESC");
...
app.get("/api/users", async (req, res) => {
    try {
        const [users] = await mainDb.query("SELECT id, display_name, email, primary_role as role, is_active, 'client' as source_table FROM users WHERE deleted_at IS NULL");
        const [admins] = await mainDb.query("SELECT id, display_name, email, admin_level as role, is_active, 'admin' as source_table FROM admin_users WHERE deleted_at IS NULL");
        res.json({ success: true, users: [...users, ...admins] });
    } catch (err) { res.status(500).json({ success: false }); }
});

// --- IDENTITY MANAGEMENT (Identity Hub) ---
const CLIENT_IDENTITY_FIELDS = ['first_name','last_name','email','phone_number','alt_phone','physical_address','id_number','expertise','private_notes','emergency_contact_name','emergency_contact_phone','manual_projects','mission_briefing','department','is_active'];
const ADMIN_IDENTITY_FIELDS = ['first_name','last_name','email','phone_number','alt_phone','physical_address','id_number','expertise','private_notes','emergency_contact_name','emergency_contact_phone','manual_projects','mission_briefing','department','is_active'];
...
app.get("/api/admin/users/:id", authenticateAdmin, async (req, res) => {
    ...
    try {
        if (roleType === 'admin') {
            const [rows] = await mainDb.query(
                ...
                [req.params.id]);
            if (!rows.length) return res.status(404).json({ success: false, message: "Identity not found" });
    ...
    } catch (err) { res.status(500).json({ success: false, message: "Protocol Error" }); }
});
...
app.put("/api/admin/users/:id", authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        const fields = pickIdentityFields(body, roleType === 'admin' ? ADMIN_IDENTITY_FIELDS : CLIENT_IDENTITY_FIELDS);
        if (body.password) {
        ...
        await mainDb.query(`UPDATE ${table} SET ? WHERE id = ? AND deleted_at IS NULL`, [fields, req.params.id]);
        res.json({ success: true });
    } catch (err) {
        if (err && err.code === 'ER_DUP_ENTRY') return res.status(409).json({ success: false, message: "Email already registered to another identity" });
        res.status(500).json({ success: false, message: "Protocol Error" });
    }
});
...
app.delete("/api/admin/users/:id", authenticateAdmin, async (req, res) => {
    ...
    try {
        if (roleType === 'admin' && Number(req.params.id) === Number(req.adminId)) {
            return res.status(400).json({ success: false, message: "You cannot terminate your own admin node." });
        }
        const table = roleType === 'admin' ? 'admin_users' : 'users';
        const [result] = await mainDb.query(
            `UPDATE ${table} SET deleted_at = NOW(), is_active = 0, deleted_by = ? WHERE id = ? AND deleted_at IS NULL`,
            [req.adminId, req.params.id]);
        if (!result.affectedRows) return res.status(404).json({ success: false, message: "Identity not found or already terminated" });
        res.json({ success: true });
    } catch (err) { res.status(500).json({ success: false, message: "Protocol Error" }); }
});
...
app.post("/api/admin/users/:id/whatsapp-password-reset", authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        const [result] = await mainDb.query(
            ...
            [hash, req.params.id]);
        if (!result.affectedRows) return res.status(404).json({ success: false, message: "Identity not found" });
        res.json({ success: true, tempPassword, message: "Temp password generated — relay it to the identity via WhatsApp or a secure channel." });
    } catch (err) { res.status(500).json({ success: false, message: "Protocol Error" }); }
});
...
app.post("/api/users/admin-create", authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        const shared = [
            body.department || null, body.mission_briefing || null,
            body.phone_number || null, body.alt_phone || null,
            body.physical_address || null, body.id_number || null,
            body.expertise || null, body.private_notes || null,
        ...
        if (err && err.code === 'ER_DUP_ENTRY') return res.status(409).json({ success: false, message: "Identity Conflict — email already registered" });
        res.status(500).json({ success: false, message: "Protocol Error" });
    }
});
...
app.get("/api/admin/dashboard", async (req, res) => {
    try {
        const [[adminCount]] = await mainDb.query('SELECT COUNT(*) as count FROM admin_users WHERE is_active = 1 AND deleted_at IS NULL');
        const [[userCount]] = await mainDb.query('SELECT COUNT(*) as count FROM users WHERE is_active = 1 AND deleted_at IS NULL');
        const [[activeProjects]] = await mainDb.query("SELECT COUNT(*) as count FROM user_projects WHERE status IN ('in-progress', 'active') AND deleted_at IS NULL");
        const [[pendingApprovals]] = await mainDb.query("SELECT COUNT(*) as count FROM user_projects WHERE status IN ('planning', 'pending') AND deleted_at IS NULL");
        let recentActivity = [];
        try {
            const [logs] = await mainDb.query("SELECT action_description as action, timestamp FROM activity_logs ORDER BY timestamp DESC LIMIT 8");
            recentActivity = logs;
        } catch (logErr) { recentActivity = []; }
        res.json({
            success: true,
            dashboard: {
                userCounts: { total: adminCount.count + userCount.count, verified: 0, live: 0, total_active_projects: activeProjects.count },
...
app.get("/api/admin/budget-overview", async (req, res) => {
    try {
        const [fn] = await mainDb.query("SELECT COALESCE(SUM(CASE WHEN entry_type IN ('income','invoice_payment') THEN amount ELSE 0 END),0) as revenue, COALESCE(SUM(CASE WHEN entry_type='expense' THEN amount ELSE 0 END),0) as expenses FROM accounting_entries WHERE deleted_at IS NULL AND payment_status='completed'");
        const r = fn[0]?.revenue || 0, e = fn[0]?.expenses || 0;
        res.json({ success: true, data: { revenue: r, expenses: e, net_income: r - e, spent: e, remaining: r - e } });
...
app.get("/api/admin/pending-approvals", async (req, res) => {
    try {
        const [rows] = await mainDb.query("SELECT id, project_name as name, client_name, status, created_at FROM user_projects WHERE status IN ('planning', 'pending') AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 10");
...
app.get("/api/admin/team", async (req, res) => {
    try {
        const [rows] = await mainDb.query("SELECT id, name, role, department, is_active FROM team_members ORDER BY name ASC");
        res.json({ success: true, team: rows });
    } catch (err) { res.json({ success: true, team: [] }); }
});
...
app.get("/api/admin/settings", async (req, res) => {
    try {
        ...
        const m = {}; settings.forEach(s => { m[s.setting_key] = s.setting_value; });
...
app.get("/api/admin/search", async (req, res) => {
    const { q } = req.query; if (!q) return res.json({ success: true, results: [] });
    const term = `%${q}%`;
    try {
        const [projects] = await mainDb.query("SELECT id, project_name as title, 'project' as type, '/admin/projects' as link FROM user_projects WHERE project_name LIKE ? LIMIT 5", [term]);
        const [users] = await mainDb.query("SELECT id, display_name as title, 'user' as type, CONCAT('/admin/users/detail/', id, '/client') as link FROM users WHERE display_name LIKE ? LIMIT 5", [term]);
        res.json({ success: true, results: [...projects, ...users] });
    } catch (err) { res.json({ success: true, results: [] }); }
});
...
app.post("/invoices", async (req, res) => {
  const { project_id, title, invoice_type, tax_type, tax_rate, issue_date, due_date, client_name, client_email, client_phone, notes, subtotal, items, currency, created_by } = req.body;
  try {
    ...
    const due = due_date || new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
    ...
    const [result] = await mainDb.query(
      `INSERT INTO invoices (project_id, invoice_type, invoice_number, title, subtotal, tax_rate, currency, status, issue_date, due_date, client_name, client_email, client_phone, notes, created_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?, ?, NOW())`,
      [project_id || null, type, invoiceNumber, title || 'Invoice', subtotal || 0, taxRate, currency || 'KES', issue_date || today.toISOString().split('T')[0], due, client_name || 'Client', client_email || '', client_phone || '', notes || '', userId]
    );
...
app.post("/invoices/:id/send", async (req, res) => {
  try {
    await mainDb.query("UPDATE invoices SET status = 'sent', email_sent = 1, email_sent_at = NOW() WHERE id = ?", [req.params.id]);
...
app.post("/api/accounting/entries", async (req, res) => {
  const { entry_type, amount, description, category, project_id, reference, transaction_date, created_by } = req.body;
  try {
    // Get a valid user id for created_by (check users table first for FK constraints)
    let userId = created_by || null;
    if (!userId) {
    ...
    if (!pid) {
      ...
      if (projects.length > 0) {
        ...
        const [result] = await mainDb.query("INSERT INTO user_projects (user_id, project_name, status, created_at) VALUES (?, 'General', 'in_progress', NOW())", [userId]);
        pid = result.insertId;
      }
    }
    ...
    const [result] = await mainDb.query(
      `INSERT INTO accounting_entries (entry_type, amount, description, category, project_id, transaction_reference, transaction_date, created_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
      [entry_type || 'expense', amount || 0, desc, category || 'General', pid, reference || '', transaction_date || today, userId]
    );
...
app.get("/api/accounting/overview", async (req, res) => {
  try {
    const [incomeRows] = await mainDb.query("SELECT COALESCE(SUM(amount), 0) as total FROM accounting_entries WHERE entry_type = 'income'");
    const [expenseRows] = await mainDb.query("SELECT COALESCE(SUM(amount), 0) as total FROM accounting_entries WHERE entry_type = 'expense'");
...
app.get("/api/contact-forms", async (req, res) => {
  try {
    const [messages] = await mainDb.query("SELECT * FROM contact_forms ORDER BY created_at DESC LIMIT 50");
    res.json({ success: true, contacts: messages });
  } catch (err) { res.json({ success: true, contacts: [] }); }
});

// M-PESA TRANSACTIONS
app.get("/mpesa/transactions", async (req, res) => {
  try {
    const [transactions] = await mainDb.query("SELECT * FROM mpesa_transactions ORDER BY created_at DESC LIMIT 50");
    res.json({ success: true, transactions });
  } catch (err) { res.json({ success: true, transactions: [] }); }
});
...
app.post("/mpesa/stkpush", async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, id: result.insertId, message: "STK push initiated. Enter PIN on your phone." });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});
...
app.post("/mpesa/send-money/record", async (req, res) => {
  const {
    mpesa_receipt, transaction_id, phone_number, amount, client_id, client_name, client_email,
    project_id, invoice_id, company_id, account_reference, notes, created_by
  } = req.body;

  try {
    // Validate required fields
    if (!mpesa_receipt || !phone_number || !amount) {
      return res.status(400).json({ success: false, error: "M-Pesa receipt, phone number, and amount are required" });
    }
...
app.get("/mpesa/send-money", async (req, res) => {
  try {
    ...
    if (client_id) { query += " AND mt.client_id = ?"; params.push(client_id); }
    if (project_id) { query += " AND mt.project_id = ?"; params.push(project_id); }
    if (invoice_id) { query += " AND mt.invoice_id = ?"; params.push(invoice_id); }
    if (company_id) { query += " AND mt.company_id = ?"; params.push(company_id); }
...
app.get("/api/companies", async (req, res) => {
  try {
    const [companies] = await mainDb.query("SELECT id, name, slug, contact_email, contact_phone FROM companies WHERE deleted_at IS NULL ORDER BY name LIMIT 50");
    res.json({ success: true, companies });
  } catch (err) { res.json({ success: true, companies: [] }); }
});
...
app.get("/api/pdf/completion/:recordType/:id", async (req, res) => {
  try {
    ...
    let data = null;
    if (recordType === 'invoice') [data] = await mainDb.query("SELECT * FROM invoices WHERE id = ?", [id]);
    else if (recordType === 'project') [data] = await mainDb.query("SELECT * FROM projects WHERE id = ?", [id]);
    else if (recordType === 'quote') [data] = await mainDb.query("SELECT * FROM quotes WHERE id = ?", [id]);
    if (!data || data.length === 0) return res.status(404).json({ success: false, error: "Record not found" });
    res.json({ success: true, data: data[0], message: "PDF generation endpoint (PDF library integration required)" });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});

// Start Strategic Relay
app.listen(PORT, '0.0.0.0', () => console.log(`[GSS MASTER RELAY] Node Online on Port ${PORT} | Primary Identity: admin_users`));
