...
// ---- Payment instructions ----
    ...
    const boxH = 58;
    doc.rect(M, y, CW, boxH).fill("#EFF6FF");
  ...
  const metaRows = [
    ...
    .map(
      ([k, v]) =>
        ...
        `<td style="padding:6px 0;font-size:12px;font-weight:700;color:#00122B;text-align:right;">${esc(v)}</td></tr>`
    )
    .join("");
  ...
  const rowsHtml = items
    .map(
      (it, i) =>
        ...
        <td style="padding:9px 8px;font-size:12px;font-weight:700;color:#00122B;text-align:right;border-bottom:1px solid #eef2f7;">${money(it.line_total, currency)}</td>
      </tr>`
    )
    .join("");
const totalsRows = `
    <tr><td style="padding:5px 8px;font-size:11px;color:#64748b;">Subtotal</td>
        <td style="padding:5px 8px;font-size:12px;font-weight:700;color:#00122B;text-align:right;">${money(subtotal, currency)}</td></tr>
    ${
      ratePct > 0
        ? `<tr><td style="padding:5px 8px;font-size:11px;color:#64748b;">${esc(taxLabel(ratePct) || `Tax (${ratePct}%)`)}</td>
        <td style="padding:5px 8px;font-size:12px;font-weight:700;color:#00122B;text-align:right;">${money(taxAmount, currency)}</td></tr>`
        : ""
    }
    <tr><td style="padding:10px 8px 4px;border-top:2px solid #00122B;font-size:12px;font-weight:800;color:#00122B;text-transform:uppercase;">TOTAL</td>
        <td style="padding:10px 8px 4px;border-top:2px solid #00122B;font-size:18px;font-weight:800;color:#002D62;text-align:right;">${money(total, currency)}</td></tr>`;
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#eef2f7;padding:24px 8px;">
    <tr><td align="center">
      <table width="620" cellpadding="0" cellspacing="0" style="width:620px;max-width:100%;background:#ffffff;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;">
        <tr><td colspan="2" style="height:6px;background:linear-gradient(90deg,#00122B,#002D62);font-size:0;line-height:0;">&nbsp;</td></tr>
        <tr>
          <td style="background:#00122B;color:#ffffff;padding:26px 30px;">
            <div style="font-size:9px;color:#93C5FD;font-weight:700;letter-spacing:3px;text-transform:uppercase;">The Greggory Systems</div>
          ...
          <td align="right" style="background:#00122B;color:#ffffff;padding:26px 30px;width:200px;">
            <div style="font-size:30px;font-weight:900;color:#93C5FD;letter-spacing:3px;">${docTitle}</div>
        ...
        <tr>
          <td style="padding:22px 30px 4px;width:50%;vertical-align:top;">
            ...
            <div style="font-size:15px;font-weight:800;color:#00122B;margin-top:6px;">${esc(document.client_name || "Client")}</div>
...
<tr>
          <td colspan="2" style="padding:18px 30px 8px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
              <tr style="background:#00122B;color:#ffffff;">
        ...
        <tr>
          <td style="padding:10px 30px 24px;vertical-align:top;width:50%;">
            <div style="background:#EFF6FF;border-left:3px solid #002D62;border-radius:6px;padding:12px 14px;margin-top:4px;">
              <div style="font-size:9px;font-weight:800;color:#002D62;letter-spacing:1px;text-transform:uppercase;">Payment Instructions</div>
              ${payLines
                .map(
                  (l, i) =>
                    `<div style="font-size:11px;color:${i === 0 ? "#00122B" : "#64748b"};font-weight:${i === 0 ? 700 : 400};margin-top:2px;">${esc(l)}</div>`
                )
                .join("")}
              ${document.title ? `<div style="font-size:11px;color:#64748b;margin-top:8px;"><strong style="color:#00122B;">Reference:</strong> ${esc(document.title)}</div>` : ""}
            </div>
</td>
          <td style="padding:10px 30px 24px;vertical-align:top;width:50%;">
...
${
          notes
            ? `<tr><td colspan="2" style="padding:14px 30px 18px;font-size:11px;color:#475569;border-top:1px solid #eef2f7;"><strong style="color:#00122B;">Notes &amp; Terms</strong><br/>${esc(notes)}</td></tr>`
            : ""
        }
        <tr>
