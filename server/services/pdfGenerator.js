...
function buildCompletionDoc(doc, recordType, r, { includeLineItems, title, subtitle }) {
  const primary = "#00122B"; // slate-900
  const accent = "#002D62";  // blue-700
  const gold = "#eab308";
  ...
  doc.font("Helvetica-Bold").fontSize(10).fillColor("#00122B").text("Record Details", 65, detailY + 6);
  ...
  if (totals.length > 0) {
    ...
    doc.save().rect(doc.page.width - 50 - boxW, boxY, boxW, boxH).fill("#EFF6FF").stroke("#BFDBFE").restore();
    doc.font("Helvetica-Bold").fontSize(9).fillColor("#002D62").text("Amounts", doc.page.width - 50 - boxW + 12, boxY + 8);

    let ty = boxY + 26;
    for (const [label, value] of totals) {
      doc.font("Helvetica").fontSize(9).fillColor("#00122B").text(label, doc.page.width - 50 - boxW + 12, ty);
      doc.font("Helvetica-Bold").fontSize(9).fillColor("#002D62").text(value, doc.page.width - 50 - 8, ty, { align: "right" });
      ty += 18;
    }
  }
  ...
  doc.font("Helvetica-Bold").fontSize(12).fillColor("#00122B").text(FIRM_LEGAL_NAME, 50, sigY + 20);
...
function drawLineItemsTable(doc, items, r) {
  doc.font("Helvetica-Bold").fontSize(10).fillColor("#00122B").text("Service Items", 50, doc.y + 15);
