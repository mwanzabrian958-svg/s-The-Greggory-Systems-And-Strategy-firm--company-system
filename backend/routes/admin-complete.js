...
function requireAdminSession(req, res, next) {
  ...
  if (!m) return res.status(401).json({ success: false, message: 'Admin authentication required' });
...
router.get('/budget-overview', requireAdminSession, async (req, res) => {
  try {
    const [pb] = await db
      .promise()
      .query(
...
router.get('/search', requireAdminSession, async (req, res) => {
  try {
    ...
    const [tk] = await db
      .promise()
      .query(
        "SELECT 'task' as type,id,task_name as title,status,priority,CONCAT('/admin/projects/',project_id,'/tasks') as link FROM project_tasks WHERE (task_name LIKE ? OR task_description LIKE ?) AND deleted_at IS NULL LIMIT ?",
        [t, t, l],
      );
...
router.post('/team', requireAdminSession, async (req, res) => {
  try {
    ...
    const [r] = await db
      .promise()
      .query(
        'INSERT INTO team_members (name,role,department,description,is_active,created_at) VALUES (?,?,?,?,1,NOW())',
        [name, role || 'member', department || null, description || null],
      );
...
router.get('/crm-telemetry', requireAdminSession, async (req, res) => {
  try {
...
router.get('/node-settings', requireAdminSession, async (req, res) => {
  try {
...
router.get('/ledger', async (req, res) => {
  try {
    ...
    res.json({ success: true, entries: e });
  } catch (err) {
    res.status(500).json({ success: false, message: 'Failed' });
  }
});
router.get('/mpesa/transactions', async (req, res) => {
  try {
    ...
    res.json({ success: true, transactions: r });
  } catch (err) {
    res.status(500).json({ success: false, message: 'Failed' });
  }
});
