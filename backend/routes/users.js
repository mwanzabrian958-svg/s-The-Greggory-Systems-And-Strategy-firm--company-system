 ...
 * /api/users/test:
 *   get:
 *     summary: Verify users router is loaded
 *     tags: [Users]
 *     responses:
 *       200:
...
router.post(
  ...
  authEndpointValidator('user', 'users'),
  async (req, res) => {
    try {
      ...
      const [existing] = await db
        .promise()
        .query(
          'SELECT id, deleted_at, password_hash, first_name, last_name, display_name FROM users WHERE email = ? LIMIT 1',
          [email],
        );
      ...
      if (existing.length > 0) {
        ...
        if (existingUser.deleted_at) {
          await db
            .promise()
            .query('UPDATE users SET deleted_at = NULL, is_active = TRUE WHERE id = ?', [
              existingUser.id,
            ]);
        }
      ...
      if (profilePhotoBase64) {
        ...
        const base64Data =
          commaIdx > -1 ? profilePhotoBase64.slice(commaIdx + 1) : profilePhotoBase64;
      ...
      await createNotification(
        ...
        'system',
        'Account Initialized',
        'Welcome to the tactical portal.',
        'normal',
      );
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await db
        .promise()
        .query('UPDATE users SET last_login_at = NOW(), last_login_ip = ? WHERE id = ?', [
          req.ip,
          user.id,
        ]);
...
router.get('/client-dashboard', authenticateUser, async (req, res) => {
  ...
  try {
    const [users] = await db
      .promise()
      .query(`SELECT * FROM users WHERE id = ? AND deleted_at IS NULL`, [id]);
    if (users.length === 0)
    ...
    const [projects] = await db
      .promise()
      .query(`SELECT * FROM user_projects WHERE user_id = ? AND deleted_at IS NULL`, [id]);
    const [invoiceRows] = await db
      .promise()
      .query(
        `SELECT pi.*, up.project_name FROM project_invoices pi JOIN user_projects up ON up.id = pi.project_id WHERE up.user_id = ?`,
        [id],
      );
    ...
    const dashboard = {
      user: {
        ...
        has_photo: !!user.profile_photo_blob,
        profilePhotoData: dashboardPhotoData,
      },
  ...
  try {
    ...
    await db
      ...
      .query(
        ...
        [display_name, phone_number, userId],
      );
...
router.get('/notifications/:id/attachment', authenticateUser, async (req, res) => {
  ...
  try {
    const [rows] = await db
      .promise()
      .query(
        'SELECT attachment_data, attachment_type, attachment_name FROM notifications WHERE id = ? AND user_id = ?',
        [noteId, userId],
      );
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${item.attachment_name || 'download'}"`,
    );
